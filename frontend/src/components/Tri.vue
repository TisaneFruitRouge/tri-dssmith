...
export default {
  ...
  setup()
  {
    ...
    onMounted(()=>{
      axios.get('http://localhost:8000/api/tris/')
        .then((response)=>{
          data = response.body;
        })
    })
    ...
    let postTri = ()=>{
      if (of.value=="" || symbole.value=="" || client.value!=="" || defaut.value!=="" || a_trier.value>0){
        champs_non_remplis.value = true;
      }
      else { champs_non_remplis.value = false; } 
      if (mauvaises.value+bonnes.value !== a_trier.value)
      {
        erreur_bons_mauvais.value = true;
      }
      else { erreur_bons_mauvais.value = false; } 
